...
if series_name == "Custom (User-defined)":
    ...
    if st.session_state.custom_confirmed:
        ...
        # Ranges for guidance
        A10_min, A10_max = 0.01, 1000
        T_min, T_max = 1e-6, 1e10  # days (converted later)

        A10 = st.number_input(
            "Initial parent activity A₀ (relative)",
            min_value=0.0, max_value=1e6, value=1.0, step=0.1
        )
        if not (A10_min <= A10 <= A10_max):
            st.warning(f"Initial activity A₀ should be between {A10_min} and {A10_max}.")
            valid_input = False

        col1, col2, col3 = st.columns(3)
        with col1:
            T1_val = st.number_input("Parent half-life", min_value=1e-12, value=1.0)
            T1_unit = st.selectbox("Unit", ["minutes", "hours", "days", "years"], key="T1")
        with col2:
            T2_val = st.number_input("Daughter half-life", min_value=1e-12, value=0.1)
            T2_unit = st.selectbox("Unit", ["minutes", "hours", "days", "years"], key="T2")
        with col3:
            T3_val = st.number_input("Granddaughter half-life", min_value=1e-12, value=0.01)
        ...
        # Check ranges
        for name, T in zip(["Parent", "Daughter", "Granddaughter"], [T1, T2, T3]):
            if not (T_min <= T <= T_max):
                st.warning(f"{name} half-life is out of reasonable range (approx {T_min}–{T_max} days).")
                valid_input = False
...
# ---------- Preset decay series ----------
elif series_name != "Custom (User-defined)":
    st.session_state.custom_confirmed = False
...
if "A10" in locals() and valid_input:
