...
def bateman_chain(A10, T1, T2, T3):
    ...
    t_max = max(5 * max(T1, T2, T3), 10 * T1)  # Ensure parent half-life is covered
...
# ---------- Preset decay series ----------
decay_series = {
    "Ra-226 (Secular)": {"equilibrium": "Secular","parent": ("Ra-226", 1600, "years"),
                                    "daughter": ("Rn-222", 3.8, "days"), "granddaughter": ("Po-218", 3.1, "minutes"),
                                    "stable": "Pb-206"},
    "Th-232 (Secular)": {"equilibrium": "Secular","parent": ("Th-232", 1.4e10, "years"),
                                    "daughter": ("Ra-228", 5.75, "years"), "granddaughter": ("Ac-228", 6.15, "hours"),
                                    "stable": "Pb-208"},
    "U-238 (Secular)": {"equilibrium": "Secular","parent": ("U-238", 4.5e9, "years"),
                                    "daughter": ("Th-234", 24, "days"), "granddaughter": ("Pa-234", 6.7, "hours"),
                                    "stable": "Pb-206"},
    "Mo-99 (Transient)": {"equilibrium": "Transient","parent": ("Mo-99", 66, "hours"),
                                     "daughter": ("Tc-99m", 6, "hours"), "granddaughter": ("Tc-99", 2.1e5, "years"),
                                     "stable": "Ru-99"},
    "Ce-144 (Transient)": {"equilibrium": "Transient","parent": ("Ce-144", 284.9, "days"),
                                      "daughter": ("Pr-144", 17.3, "minutes"), "granddaughter": ("Pr-144m", 7.2, "minutes"),
                                      "stable": "Nd-144"},
    "Cs-137 (Transient)": {"equilibrium": "Transient","parent": ("Cs-137", 30.05, "years"),
...
# Only show plot type selector for preset series
if series_name != "Custom":
    ...
    plot_mode = "Combined"  # For custom, always show all three

# ---------- Define equilibrium type ----------
def equilibrium_type(T1, T2, T3):
    # Consider the shortest daughter half-life as reference
    shortest_daughter = min(T2, T3)
    if T1 >= 100 * shortest_daughter:
        return "Secular"
    elif T1 > shortest_daughter:
        return "Transient"
    else:
        return "Transient"  # If parent < daughter, still transient in this simplified model
...
if A10 is not None:
    ...
    # Hide daughter & granddaughter if preset and "Parent only" selected
    if series_name != "Custom" and plot_mode == "Parent only":
